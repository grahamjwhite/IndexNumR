...
\end{equation}
and use these to calculate the quantities,

\begin{equation}
  q_{tn} = \frac{e_{tn}s_{tn}}{p_{tn}}
\end{equation}

*IndexNumR* provides the function `CESData` to produce CES datasets as above for any elasticity of substitution $\sigma$, using the prices, $\alpha$, and expenditure values assumed in [@df:2017], reproduced below.
...
: Prices and expenditure for sample CES datasets 
...
\end{equation*}
where $D^{t}$ is equal to 1 in period $t$ and 0 in period $t-1$, and $D_{i}$ is equal to 1 if the product is product $i$ and 0 otherwise. The equation is estimated by weighted least squares (as suggested by [@diewert:2005b]), where the weights are the product expenditure shares.
...
However, this is a biased estimate [@kennedy:1981], so *IndexNumR* calculates the following,
...
Multilateral index number methods use data from multiple periods to compute each term in the index. *IndexNumR* provides the functions `GEKSIndex` and `GKIndex` to use the GEKS or Geary-Khamis multilateral index number methods respectively. 
...
Once the $b$ vector has been calculated, the price levels can be computed from the GK equations above. As with other multilateral methods applied to the time-series context, the index must be extended in real-time as new observations become available. Therefore, the GK index is calculated over some window of time $w$, and methods to extend the index beyond period $w$ are discussed below.  

### Extending multilateral indexes

Extending a multilateral index can be done in a multitude of ways. Statistical agencies generally do not revise price indices like the consumer price index, so the methods offered by *IndexNumR* to extend multilateral indexes are methods that do not lead to revisions. More specifically, these are called *splicing methods* and the four options available are the *movement*, *window*, *half* and *mean splice*. The idea is that we start by moving the window forward by one period and calculate the index for the new window. There will be $w-1$ overlapping periods between the initial index and the index computed on the window that has been rolled forward one period. Any one of these overlapping periods can be used to extend the multilateral index.  
