...
* Montgomery indicator
\begin{equation}
  I(p^{t-1}, p^{t}) = \sum_{n=1}^{N} \frac{p_{n}^{1}q_{n}^{1}+p_{n}^{0}q_{n}^{0}}{log(p_{n}^{1}q_{n}^{1}) - log(p_{n}^{0}q_{n}^{0})} \times\left(\frac{p_{n}^{1}}{p_{n}^{0}}\right)
\end{equation}
...
Note that for this decomposition, the method is specified for the price indicator and `IndexNumR` uses the appropriate quantity indicator. For Bennet and Montgomery indicators the same method is used for the quantity indicator as for the price indicator. If a Laspeyres indicator is requested for the price indicator then the corresponding volume indicator is a Paasche indicator. The reverse is true if the Paasche indicator is used for prices.  
