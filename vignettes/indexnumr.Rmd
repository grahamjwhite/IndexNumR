...
\begin{equation*}
  \log{p_{n}^{t}} = \alpha_{0} + \beta_{1} D^{t} + \sum_{i = 2}^{N}\beta_{i}D_{i} + \epsilon_{i}^{t},
\end{equation*}
where $D^{t}$ is equal to 1 in period $t$ and 0 in period $t-1$, and $D_{i}$ is equal to 1 if the product is product $i$ and 0 otherwise. The equation is estimated by weighted least squares, where the weights are the product expenditure shares.
