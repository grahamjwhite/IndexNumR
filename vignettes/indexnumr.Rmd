...
Multilateral index number methods use data from multiple periods to compute each term in the index. *IndexNumR* provides the function `GEKSIndex` to use the GEKS multilateral index number method. The GEKS method is attributable to Gini, Eltito and Koves, and Szulc in the cross-sectional context. It has been adapted to the time series context by Balk, and developed further by Ivancic, Diewert and Fox. 
