...
\end{equation*}
where $q^{b}$ is the vector of quantities from a base period that is not necessarily period $t$ or $t-1$. 
...
\begin{equation*}
  P(p^{t-1},p^{t},q^{t-1},q^{t}) = A + \sqrt{A^2 + E^{t}/E^{t-1}},
\end{equation*}
where $E^{t}$ is total expenditure in period $t$, $A = (P_{L}-Q_{L})/2$, $P_{L}$ is the laspeyres price index and $Q_{L}$ is the laspeyres quantity index.
...
\end{equation}
where the term $P^{k,t}$ is the bilateral index between period $t$ and base period $k$. *IndexNumR* offers the Fisher, Tornqvist, Walsh and time-product-dummy index number methods for the index $P$ via the `indexMethod` option. The Tornqvist index method is the default. The $w\times w$ matrix of bilateral indexes is as follows,
\[P =
\begin{pmatrix}
  P^{1,1} & \cdots & P^{1,w} \\
  \vdots & \ddots & \vdots \\
