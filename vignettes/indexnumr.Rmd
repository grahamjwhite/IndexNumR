...
\begin{equation}
  P_{GEKS}^{w+1} = P_{GEKS}^{w} \times \frac{P_{NEW}^{w+1}/P_{NEW}^{2}}{P_{OLD}^{w}/P_{OLD}^{1}}
\end{equation}
...
\begin{equation}
  P_{GEKS}^{w+1} = P_{GEKS}^{w} \times \left( \prod_{t=1}^{w-1} \frac{P_{NEW}^{w+1}/P_{NEW}^{t+1}}{P_{OLD}^{w}/P_{OLD}^{t}} \right)^{\frac{1}{(w-1)}} 
\end{equation}
...
The splicing methods are used in this fashion to extend the series up to period $T$. 
